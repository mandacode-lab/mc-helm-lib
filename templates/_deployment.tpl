...
Parameters:
  component  - Component name (e.g. "api")
  config     - Component configuration from values
  root       - Root context ($)
  configRefs - (optional) List of ConfigMap/Secret to inject via envFrom
    - type:     "configMap" or "secret"
    - name:     Resource name for envFrom
    - template: Template name for checksum calculation
    - params:   Parameters to pass to template
...
{{ include "mc-helm-lib.deployment" (dict
  ...
  "config" .Values.api
  "configRefs" (list
    (dict "type" "configMap" "name" "my-config" "template" "myapp.config" "params" $)
  )
  "root" $
) }}
*/}}
{{- define "mc-helm-lib.deployment" -}}
{{- $component := .component -}}
{{- $config := .config -}}
...
spec:
  ...
  selector:
    matchLabels:
      {{- include "mc-helm-lib.selectorLabels" (dict "component" $component "root" $root) | nindent 6 }}
  template:
    metadata:
      annotations:
        ...
        {{- end }}
      labels:
        {{- include "mc-helm-lib.labels" (dict "component" $component "root" $root) | nindent 8 }}
    ...
    spec:
      ...
      imagePullSecrets:
        {{- toYaml . | nindent 8 }}
      {{- end }}
      serviceAccountName: {{ include "mc-helm-lib.serviceAccountName" $root }}
      securityContext:
